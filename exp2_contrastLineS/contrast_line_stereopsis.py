...
use_scr = 1
rept = 5
#test_eye = -1  # If test_x = 1, rds have uncross disparity
...
file_num = int((len(files) - 1) / 2) # If DS.store is in directory, int((len(files) - 1) / 2)
ran_num = list(range(1, file_num+1))*rept
...
def Get_results(dt):
    ...
    dtstd.append(d)
    print("--------------------------------------------------")
    print("trial: " + str(n) + "/" + str(len(ran_num)))
...
results = pd.DataFrame({"condition": ran_num,  # Store variance_A conditions
